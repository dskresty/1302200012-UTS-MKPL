...
public class TaxFunction {
	...
	private static final int PenghasilanSingleTanpaPajak = 54000000;
	private static final int PenghasilanNikahTanpaPajak = 58500000;
	private static final int PenghasilanTambahanPerAnak = 1500000;
	...
	public static int calculateTax(int monthlySalary, int otherMonthlyIncome, int numberOfMonthWorking, int deductible,
		...
		if (isMarried) {
			tax = (int) Math.round(Pajak * (((monthlySalary + otherMonthlyIncome) * numberOfMonthWorking) - deductible
					- (PenghasilanNikahTanpaPajak + (numberOfChildren * PenghasilanTambahanPerAnak))));
		} else {
			tax = (int) Math.round(Pajak * (((monthlySalary + otherMonthlyIncome) * numberOfMonthWorking) - deductible
					- PenghasilanSingleTanpaPajak));
		}
